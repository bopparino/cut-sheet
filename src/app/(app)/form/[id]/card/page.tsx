...
// TODO(card-redesign): The Replica view is now the default editor (the /form/[id]
// route redirects there). This Card view is kept as a secondary view but needs a
// dedicated design pass to match the new direction — its dense 2-col grid and
// per-section cards predate that decision. Don't invest in it until the redesign
// is scoped.
export default async function EditCutsheetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rail: RailItem[] = [
    ...
    { id: "g-docs", label: "Attachments", badge: docsCount },
  ];
    ...
    <div key={row.id}>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-6 py-3">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <Link
              ...
              target="_blank"
              className="inline-flex h-9 items-center gap-1.5 rounded-sm border border-input bg-card px-3 text-[12px] font-semibold text-foreground hover:bg-accent"
            >
      ...
      <div className="flex gap-7 px-6 py-7">
        ...
        <CutsheetForm formId={FORM_ID} action={update} className="min-w-0 flex-1 space-y-9">
          ...
          <Group id="g-docs" title="Attachments">
            <AttachmentsCard className="xl:col-span-2" cutsheetId={numeric} attachments={attachments} />
          </Group>
        </CutsheetForm>
      </div>
    </div>
